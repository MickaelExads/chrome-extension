const mediaJSON =
    [
        {
            ...
            "sources": "https://docs.evostream.com/sample_content/assets/bun33s.mp4",
            "subtitle": "00:33",
            "thumb": "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/images/BigBuckBunny.jpg",
            "title": "Trailer for Big Buck Bunny"
        },
        {
            "description": "Big Buck Bunny tells the story of a giant rabbit with a heart bigger than himself. When one sunny day three rodents rudely harass him, something snaps... and the rabbit ain't no bunny anymore! In the typical cartoon tradition he prepares the nasty rodents a comical revenge.\n\nLicensed under the Creative Commons Attribution license\nhttp://www.bigbuckbunny.org",
    ...
    const selectElement = document.getElementById('public-media-select')

    mediaJSON.forEach(media => {
        const option = document.createElement('option')
        option.value = media.sources
        option.textContent = `${media.title} - ${media.subtitle}`
        selectElement.appendChild(option)
    })

    selectElement.onchange = function(){
        const panelPublic = document.getElementById('public-panel')
        panelPublic.classList.add('d-none')
        panelPublic.classList.remove('d-flex')
        const find = mediaJSON.find((e) => e.sources === selectElement.value)
        if(find) {
            panelPublic.classList.remove('d-none')
            panelPublic.classList.add('d-flex')
            document.getElementById('public-image').src = find.thumb
            document.getElementById('public-link').value = find.sources
            document.getElementById('public-href').href = find.sources
        }
    }